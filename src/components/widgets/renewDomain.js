...
const RenewDomain = (props) => {
  ...
  useEffect(() => {
    ...
    let widget = document.getElementById("enc-renewDomain-widget");
    if (!widget) {
      // main widget
      let scriptWidget = document.createElement('script');
      scriptWidget.id = "enc-renewDomain-widget";
      scriptWidget.type = 'text/javascript';
      scriptWidget.async = true;
      let theUrl = process.env.ESB_URL + '/api/encirca/' + props.partnerId + '/widgets/renewDomain';
      scriptWidget.src = theUrl + ( theUrl.indexOf("?") >= 0 ? "&" : "?") + 'ref=' + encodeURIComponent(window.location.href);
      // embed placeholder
      let placeholder = document.getElementById("enc-renewDomain-embed");
      if (placeholder) {
  ...
  return (
	  <div id="enc-renewDomain-embed"></div>
  )
}
